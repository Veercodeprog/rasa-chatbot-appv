...
function getBotResponse(text) {
  botResponse = `<img class="botAvatar" src="./static/img/botAvatar.jpg"/><span class="botMsg">${text}</span><div class="clearfix"></div>`;
  return botResponse;
}
...
function setBotResponse(response) {
  ...
  setTimeout(() => {
    ...
    if (response.length < 1) {
      ...
      const BotResponse = `<img class="botAvatar" src="./static/img/botAvatar.jpg"/><p class="botMsg">${fallbackMsg}</p><div class="clearfix"></div>`;
        ...
        if (Object.hasOwnProperty.call(response[i], "text")) {
          if (response[i].text != null) {
            ...
            if (
              ...
              if (!botResponse) {
                botResponse = `<img class="botAvatar" src="./static/img/botAvatar.jpg"/><p class="botMsg">${response[i].text}</p><div class="clearfix"></div>`;
              }
            }
...
async function send(message) {
  ...
  $.ajax({
    ...
    data: JSON.stringify({ message, sender: uuidv4() }),
 ...
 * & sends the `/restart` event to the Rasa server
 */
